...
def local_search(best, cities, max_no_impr, neighborhood):
    ...
    while count < max_no_impr:
        candidate = {}
        candidate['vector'] = [el for el in best['vector']]
...
def variable_neighborhood_search(cities, neighborhoods, max_no_impr):
    best = {}
    ...
    while count < max_no_impr:
        for neighborhood_size in neighborhoods:
            candidate = {}
            candidate['vector'] = [el for el in best['vector']]
            ...
            candidate = local_search(
                candidate, cities, max_no_impr, neighborhood_size + count)
            if candidate['cost'] < best['cost']:
                best = candidate
                count = 0
                break
...
if __name__ == '__main__':
    ...
    max_no_impr = 30
    neighborhoods = list(range(1, 20))

    best = variable_neighborhood_search(berlin52, neighborhoods, max_no_impr)
